...
/quit.list {
  set %tmp.LQ1 1
  $report($null,****************************).active
  while (%tmp.LQ1 <= $server(0)) {
    $report(Quit List,$null,$null,%tmp.LQ1,$server(%tmp.LQ1)).active
    inc %tmp.LQ1
    if (%tmp.LQ1 > $server(0)) { break }
  }
  unset %tmp.LQ1
  $report($null,****************************).active
  $report(Pick a Server,/quit 1,2,3,4).active
  $report($null,$null,NOT YET WORKING DO NOT USE).active
  return
}
/back {
