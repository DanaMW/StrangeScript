...
on 1:SOCKREAD:Spy*:{
  ...
  if ($gettok(%spy.readline,2,32) == NOTICE) { privmsg $me $gettok(%server.spy. [ $+ [ $remove($sockname,Spy) ] ] ,1,44) $remove($gettok(%spy.readline,1,33),:) $+ @ $+ $remove($gettok(%spy.readline,3,32),:) $+ : Notice - $remove($gettok(%spy.readline,4-,32),:) }
