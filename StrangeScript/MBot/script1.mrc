...
on *:TEXT:*:#: {
  ...
  if ($level($address($nick,4)) = 5) || ($level($address($nick,1)) = 5) || ($level($address($nick,4)) = 4) || ($level($address($nick,1)) = 4) {
    ...
    ;#.hop Format: .HOP <#room> <-Aa/ADD|-D/DEL|-L/LIST|-W/WIPE> [<nick>] (Configures the join auto half ops settings.)
    ...
    if ($strip($1) == .menu) { if ($nick != %boss. [ $+ [ $network ] ]) { $pointer $report(Error,NoGoMoJo,This is a boss only command) | halt } | .enable #Menu | $point $report(Spy Menu,ON) | menu # | halt }
    ...
    ;#.tease Format: .tease (.)
    if ($strip($1) == .tease) {
      if ($nick != %boss. [ $+ [ $network ] ]) { notice $nick $report(Nice Try,$null,This is a boss only command) | halt }
      /rt # $2
      halt
    }
