...
on *:TEXT:*:#: {
  ...
  if ($strip($1) == .autostart) {
    ...
    if ($2 == -S) || ($2 == SHOW) || ($2 == -L) || ($2 == LIST) {
      ...
      $point $report(.autostart <ON|OFF|-A/ADD|-D/DEL/DELETE|-S/SHOW|-L/LIST>)
      halt
    }
    if ($2 == -A) || ($2 == ADD) {
  ...
  ;#.boot Format: .boot [#channel] <nick> (Kicks the given nick out of the active room or [#channel].)
  if ($strip($1) == .boot) {
  ...
  ;#.talk Format: .talk [ON|OFF|-A/ADD|-D/DEL|-L/LIST|-S/STATUS|-H/HELP] <#room> (Controls Interactive Speach for the given <#room>.)
  if ($strip($1) == .talk) {
    ...
    if ($2 == OFF) {
      set %talk.on. [ $+ [ $network ] ] OFF
      if ($3 == $null) { $point $report(Interactive Speach,$null,Turned,%talk.on. [ $+ [ $network ] ]) | halt }
      if ($3 != $null) { set %talk.on. [ $+ [ $network ] ] ON | set %talk.room. [ $+ [ $network ] ] $remtok(%talk.room. [ $+ [ $network ] ],$3,44) }
      $point $report(Interactive Speach,$null,Turned,%talk.on. [ $+ [ $network ] ])
      $point $report(Active Rooms,$null,$null,$replace(%talk.room. [ $+ [ $network ] ],$chr(44),$chr(32)))
      halt
    }
    if ($2 == -L) || ($2 == LIST) {
      if (%talk.on. [ $+ [ $network ] ] == $null) { set %talk.on. [ $+ [ $network ] ] OFF }
      $point $report(Interactive Speach,$null,Turned,%talk.on. [ $+ [ $network ] ])
      $point $report(Active Rooms,$null,$null,$replace(%talk.room. [ $+ [ $network ] ],$chr(44),$chr(32)))
      halt
    }
