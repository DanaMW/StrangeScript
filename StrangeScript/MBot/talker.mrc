on *:TEXT:*:#:{
  ...
  if ($nick == $chan(#)) { halt }
  if ($nick == %boss) { if (*shutup* iswm $strip($1-)) { .msg # $report(Speach interaction,$null,$null,Off) | .unload -rs talker.ini | halt  } }
  ...
  if (*nigger* iswm $strip($1-)) { .msg # $read $mircdirtext\nigger.txt | halt }
  ...
  if (*good* iswm $strip($1-)) { .msg # You know I'm good $nick | halt }
  ...
  if (*butt* iswm $strip($1-)) { .msg # Kiss my butt $nick | halt }
  if (*night* iswm $strip($1-)) { .msg # Good night $nick. | halt }
  if (*yes* iswm $strip($1-)) { .msg # No Way | halt }
  if (*bot* iswm $strip($1-)) { .msg # $read $mircdirtext\bot.txt | halt }
  if (*good night* iswm $strip($1-)) { .msg # Good night $nick. | halt }
  if (*rofl* iswm $strip($1-)) { .msg # Get up off the floor $nick | halt }
  if (*lmao* iswm $strip($1-)) { .msg # thats alot of ass to laugh off $nick | halt }
  if (*opinion* iswm $strip($1-)) { .msg # $+ Pot should be legal | halt }
  if (*kewl* iswm $strip($1-)) { .msg # Thanks I think I'm kewl too $nick | halt }
  if (*cool* iswm $strip($1-)) { .msg # I am the coolest! | halt }
  if (*bye* iswm $strip($1-)) { .msg # $read $mircdirtext\bye.txt | halt }
  if (*yep* iswm $strip($1-)) { .msg # yep I'm right $nick | halt }
  if (*coo iswm $strip($1-)) { .msg # Wheres the fucking L $nick | halt }
  if (*stoned* iswm $strip($1-)) { .msg # I'm always stoned | halt }
  if (*script* iswm $strip($1-)) { .msg # I write my own damn scripts $nick | halt }
  if (*sleep* iswm $strip($1-)) { .msg # You need some sleep $nick, you look like shit. | halt }
  if (*yup* iswm $strip($1-)) { .msg # Nope | halt }
  if (*arse* iswm $strip($1-)) { .msg # $nick is an arse fucker | halt }
