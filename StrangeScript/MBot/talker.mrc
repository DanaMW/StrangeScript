on *:TEXT:*:#:{
  if (%talk.on. [ $+ [ $network ] ] == ON) {
    ...
    if ($nick == $chan(#)) { halt }
    if ($nick == %boss. [ $+ [ $network ] ]) { if (*shutup* iswm $strip($1-)) { msg # $report(Speach interaction,$null,$null,Off) } }
    ...
    if (*addy* iswm $strip($1-)) { msg # https://danamw.github.io | halt }
    ...
    if (*question* iswm $strip($1-)) { msg # Oh great! another question from $nick | halt }
    if ($chr(63) isin $strip($1-)) { if ($rand(1,3) == 1) { msg # What is real, anyway? } | halt } 
    if (*llol* iswm $strip($1-)) { msg # lol | halt }
    if (*loll* iswm $strip($1-)) { msg # lol | halt }
    if (*lool* iswm $strip($1-)) { msg # lol | halt }
    ...
    if (*anal* iswm $strip($1-)) { msg # Studies have shown people that use the word anal like it up the pooper | halt }
    ...
    if (*stupid* iswm $strip($1-)) { msg # Holds up a mirror for $nick | halt }
    if (*shut up* iswm $strip($1-)) { msg # Harsh. | halt }
    ...
    if (hey $me iswm $strip($1-)) { msg # $read -n $mircdirtext\ME.TXT | halt }
  }
}
